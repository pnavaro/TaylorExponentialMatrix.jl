...
"""
    taylor_approximant_of_degree(A, order)

Improved Paterson Stockmeyer scheme, with Pade at end point
"""
function taylor_approximant_of_degree(A, order)
...
function expm2(A::StridedMatrix{T}) where {T<:BlasFloat}
    ...
    if normA <= theta[end] # no scaling and squaring is required.
        s = 0
        for method_selector = eachindex(m_vals)
            if normA <= theta[method_selector]
                F = taylor_approximant_of_degree(A, m_vals[method_selector])
                break
            end
        end
    else
        ...
        A = A ./ 2^s    # Scaling
        F = taylor_approximant_of_degree(A, m_vals[end])
        for i = 1:s
            F = F * F  # Squaring
        end
    end
